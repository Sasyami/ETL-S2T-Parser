...
@patch("agents.summarizer_agent._invoke")
@patch("agents.summarizer_agent.fetch_file_data")
def test_generate_analysis_texts_uses_one_llm_call(mock_fetch, mock_invoke):
    mock_fetch.return_value = {"filename": "x.xlsx", "semantic_catalog": {}}
    mock_invoke.return_value = json.dumps(
        {"summary": "Final summary.", "description": "Short description."}
    )

    result = generate_analysis_texts(1)

    assert result == ("Final summary.", "Short description.")
    mock_invoke.assert_called_once()


@patch("agents.summarizer_agent.generate_analysis_texts")
def test_generate_summary(mock_generate):
    mock_generate.return_value = ("Final summary.", "Short description.")
...
def test_summarize_file(mock_update_summary, mock_update_description, mock_generate):
    mock_generate.return_value = ("Generated summary", "Generated description")
    result = summarize_file(1, save=True)
    assert result == "Generated summary"
    mock_update_summary.assert_called_once_with(1, "Generated summary")
    mock_update_description.assert_called_once_with(1, "Generated description")
