...
- `agents/tools/routing.py` - отдельный LLM tool-router: модель получает раздельные компактные каталоги внешних data tools, skills и schemas и через `with_structured_output(ToolRoute, method="function_calling")` возвращает строго валидируемые списки; tools образуют доступную planner палитру, поэтому при неоднозначности router включает все уместные read-only возможности, не требуя минимального или взаимоисключающего выбора. Все три списка выбираются одновременно и независимо, каждый из них может быть пустым независимо от остальных, а выбранные runtime skills и фактические схемы/маппинги загружаются лениво. Внутренние действия worker `analyze` и `finish_worker` в каталог router не входят. После двух невалидных ответов router используется ограниченный общий read-only fallback по SQLite, Neo4j, Excel-значениям и семантическим описаниям; при reroute он дополняет, а не заменяет предыдущую палитру.
- `agents/chat_graph.py` - общий многошаговый LangGraph с нативным tool calling: observer возвращает structured `Observation` с отдельными `goal_satisfied`, единой консолидированной строкой `problem`, summary, фактами и ограничениями; невалидный structured observer получает до пяти повторных попыток на том же сохранённом tool result без повторного data-tool вызова, после чего worker завершается явной ошибкой. Legacy chat после planner использует responder, а изолированный worker завершается самим planner через `finish_worker(answer)` либо его обычный финальный текст без отдельного LLM-responder. В worker-режиме каждый planner-вызов получает исходную task, только последний `AIMessage.tool_calls`/`ToolMessage` обмен и одну накопительную observer-выжимку; при `goal_satisfied=false` planner отдельно получает `problem`. Первый planner-шаг видит только выбранные router data tools; после успешного полного результата ему становится доступно внутреннее действие `analyze` и точные `tool_result_ids`. Производный вывод, которого нет явным фактом в data-tool результате, требует `candidate_analysis`: observer не выполняет такую интерпретацию сам и не разрешает завершить worker до анализа. Явный вызов `analyze` запускает отдельный structured analyzer LLM, а observer критикует анализ по тем же полным источникам. Выбор skill сам analyzer не запускает; worker не выбирает результаты для UI.
