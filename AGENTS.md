...
- Runtime LLM-provider для агентной части и S2T matching настраивается через `LLM_PROVIDER`; для S2T matching используй OpenAI-compatible provider `openrouter` или `ollama`. Не добавляй молчаливые non-LLM fallback-записи: если LLM не дал валидный план, возвращай ошибку.
