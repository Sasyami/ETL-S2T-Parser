...
- `agents/worker.py` - экспериментальный изолированный read-only worker: получает только самодостаточную подзадачу, внутри выбирает tools/skills и запускает planner/tool/observer-цикл; в LLM-контексте остаётся единое ограниченное текстовое preview tool result, полные успешные результаты хранятся отдельно, а публичный worker-ответ содержит только краткий LLM-ответ и временные непрозрачные result refs с именами tools без полных данных.
- `agents/coordinator.py` - промежуточный coordinator LangGraph `plan -> materialize -> worker -> ... -> aggregate`: получает отдельно цель и ограниченный общий context, затем отдельная LLM делит цель на семантический план workers и для каждого шага фиксирует presentation `answer_only`/`full_results`. При наличии context даже единственный шаг сначала материализуется в самодостаточную task; worker получает только эту строку, без отдельного context. После выполнения всех шагов coordinator агрегирует краткие ответы и выбирает result keys для scrollable UI. Aggregator видит только общий context, краткие ответы и каталог `{result_key, tool_name}`; полные tool results в его state и LLM-контекст не передаются.
