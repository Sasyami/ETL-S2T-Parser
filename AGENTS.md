...
- `OPERATION_SQL_RISK_SCOPE_EVIDENCE_EXPERIMENT=typed_plan` включает узкую code-owned ветку для conditional-cardinality и nullable-constraint. Operation router выбирает typed execution mode native enum; последующие стадии не классифицируют запрос regex/keyword-эвристиками. Coordinator строит deterministic worker-plan, проверяет exact saved-result evidence и компилирует ответ. Cardinality compiler извлекает JOIN внешнего SELECT и не использует `WHERE`/`COALESCE` как механизм или evidence уникальности; nullable compiler читает exact mapping и role-preserving metadata. Literal arrow/file_id parsing используется только для проверки происхождения scope. Более широкие/неоднозначные запросы остаются на default agentic path; default выключен, старый prompt-mediated режим удалён и его значения отклоняются.
...
- Anti-overfit refactor удалил NL keyword/fullmatch intent classifiers и fixture-shaped SQL-пример; semantic mode теперь выбирает только structured operation-router enum. Max/Max regression run `20260910_184216` на шести RU/EN/reordered переформулировках дал candidate hard+semantic `6/6`, ноль HTTP/tool/judge errors и `-61,0%` agent tokens; это раскрытая development-проверка, typed default остаётся выключен. Подробности — `LIVE_SQL_RISK_ANTI_OVERFIT_PARAPHRASE_REPORT_2026-09-10.md`.
