...
- `sheet_skills/column_catalog.py` - строит `source_columns`/`target_columns`: специализированные листы имеют приоритет, недостающие колонки и атрибуты добираются из сырых строк S2T в `data` с учётом расположения одинаковых заголовков в source/target-блоках.
...
- `source_columns` и `target_columns` хранят связанную с S2T-парой `table_name`/`column_name` метаинформацию: `data_type`, `primary_key`, `not_null`, единое `description` и источник заполнения `metadata_source`; отсутствующие специализированные листы и значения дополняются только из сырых строк S2T в `data`.
