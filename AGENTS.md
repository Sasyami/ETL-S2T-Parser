...
- `OPERATION_SQL_RISK_SCOPE_EVIDENCE_EXPERIMENT=1` включает development-контракт точного SQL-risk scope: обязательные exact readers добавляются к worker palette, `complete` требует их принятые результаты, а endpoint scope стабилизируется в публичном ответе. Default выключен; неоднозначные dotted endpoints fail-closed не интерпретируются.
