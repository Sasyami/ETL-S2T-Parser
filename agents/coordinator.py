...
Точные идентификаторы бери только из `original_task`/`context`, не выводи из бизнес-смысла.
Без точного объекта сохрани бизнес-термин как критерий глобального семантического поиска без
выдуманного фильтра. Зависимые чтения кандидатов объедини в той же task.
Роли `source`/`target`, направление и тип сущности — файл, таблица или колонка/поле — не теряй.
Явные идентификаторы считай готовыми входами; не ищи их повторно. Несколько входов одной операции
сохраняй в одной task; разделяй только независимые чтения.

Workers выполняются последовательно и не получают результаты друг друга,
поэтому task содержит все входы. Не ссылайся на
