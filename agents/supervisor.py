...
При делегировании сформируй два принципиально разных поля: `task` и `context`.
Не используй их как две части пересказа диалога и не дублируй один и тот же
текст в обоих полях.

`task` — конкретное самодостаточное поручение, которое нужно выполнить сейчас.
Пиши его как прямую инструкцию исполнителю, а не как описание разговора,
намерение, тему, справку или план. Task должна содержать все запрошенные
результаты и действия, их зависимости и порядок, точные источники и объекты,
роли и поля, операции, условия, фильтры, сортировки, числовые ограничения,
запреты и требуемую форму ответа или отдельного показа результата. Дословно
сохраняй значимые идентификаторы, названия и значения. Не заменяй их похожими,
не обобщай конкретную операцию и не добавляй собственной предметной трактовки.
Не придумывай требования, которых нет в запросе или в устойчивых предпочтениях
пользователя. В частности, не требуй JSON, объект, список, таблицу, Markdown или
другой формат ответа по собственной инициативе; передавай требование к формату
только когда его явно задал пользователь.
Не пиши в task «пользователь хочет», «обсуждалось», «как выше», «продолжи» или
другой метатекст: coordinator должен получить именно исполнимое поручение.

Любой разовый факт из `recent_history`, без которого нельзя выполнить текущий
запрос, является частью task, а не context. Это относится к конкретному ID,
имени объекта, числу, ранее найденному значению, тексту запроса и результату
предыдущего шага. Если `current_query` однозначно ссылается на такой факт,
подставь его точное значение непосредственно в task. Не выноси необходимый
аргумент текущего действия в context и не заставляй coordinator заново
разрешать ссылку по context.
...
- текущую task или её сокращённый пересказ;
...
объект. Никогда не передавай такую неразрешённую ссылку в task. Если в
recent_history однозначно названы конкретная таблица, файл, лист, колонка,
запрос или другой объект, замени ссылку его точным названием. Например:
история «речь о таблице X», запрос «посчитай в ней строки» должны дать task
«посчитай строки в таблице X», а не task со словами «в ней».
...
Перед native call проверь `task` отдельно от `context`:
1. По одной task однозначно понятно, что именно, над чем и с какими условиями
   нужно сделать сейчас.
2. В task нет местоимений и ссылок, требующих читать историю.
3. В context остались только повторно применимые договорённости, а не входные
   данные текущей task.
4. Ни одно точное требование current_query не потеряно и не подменено.
            ...
            "parameters": {
                ...
                "properties": {
                    "task": {
                        "type": "string",
                        "description": (
                            "Исполнимое самодостаточное поручение на текущий "
                            "ход: все действия, объекты, входные факты, точные "
                            "значения, операции, условия, ограничения и форма "
                            "результата; ссылки на историю уже разрешены. Не "
                            "содержит метатекст и не зависит от context для "
                            "определения текущих входных данных."
                        ),
                    },
                    "context": {
                        "type": "string",
                        ...
                        "description": (
                            ...
                            "текущую task, разовые объекты, ID, числа, результаты "
...
def build_supervisor_graph(
    ...
    *,
    ...
    def coordinator_node(state: SupervisorGraphState) -> Dict[str, Any]:
        ...
        delegated_task = str(call["args"]["task"]).strip()
