...
def run_sql(
    ...
    """Выполнить один read-only SQL-запрос по актуальной SQLite-базе проекта.

    Используй для произвольных табличных срезов, фильтрации, подсчётов и
    агрегаций, включая строки S2T-маппинга и обычные связи source → target.
    ...
    это сценарий Neo4j. Поддерживаются SELECT, WITH и EXPLAIN QUERY PLAN.
    ...
    Args:
        query: Полный текст одного read-only SQL-запроса.
