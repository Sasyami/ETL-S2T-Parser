...
def trace_transformation_path(
    ...
    Если пользователь назвал таблицу фрагментом или без квалификатора схемы,
    выбирай одновременно search_s2t_transformations: сначала разреши по его
    результату точное полное имя, затем передай это имя в table_name данного
    tool. Не ищи сокращённое имя среди физических SQLite-таблиц и не сравнивай
    физические записи вместо построения S2T-пути.

    Результат этого tool уже содержит сохранённые шаги, правила, additional
    objects и опциональное подтверждение Neo4j. Не добавляй отдельные list/Neo4j
    tools только для повторной проверки тех же фактов. При интерпретации SQL
    различай назначение конструкций: условие WHEN внутри CASE выбирает значение
    и не фильтрует строки; строки фильтруют WHERE, HAVING и ограничивающие
    условия JOIN. UNION и UNION ALL объединяют ветви и сами не являются
    агрегацией или преобразованием значения. Несколько S2T-строк с одинаковыми
    file_id, target_table, target_field и transformation_rule, но разными
    source_field — это входы одного выражения. Сохраняй каждый исходный факт,
    но в объяснении группируй их как одну логическую трансформацию с несколькими
    входами, а не как независимые альтернативные преобразования.
