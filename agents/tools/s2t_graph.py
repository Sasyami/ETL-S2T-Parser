...
def visualize_s2t_table_graph() -> Dict[str, Any]:
    """Построить интерактивный граф связей таблиц из s2t_transformations.

    Используй только для явной просьбы показать граф, визуализацию или схему
    связей всех таблиц по глобальной таблице трансформаций. Инструмент никогда
    не принимает и не применяет file_id. Для каждой S2T-строки он разбирает SQL
