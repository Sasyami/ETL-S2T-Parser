...
def list_s2t_transformations(
    ...
    не применяет неявный file_id или последнюю загрузку.
    ...
    Если пользователь задаёт точное условие по роли таблицы, передавай значение
    отдельно в target_table или source_table. Не записывай выражение вида
    `target_table t_name` в q: q предназначен только для поиска подстроки сразу
    по всем полям.
...
def search_s2t_transformations(
    ...
    Используй для табличного поиска и просмотра фактических строк, содержащих одну известную
    подстроку. Для полного списка уникальных значений конкретного поля, особенно
    с условием по другому полю, выбирай run_sql с DISTINCT: текстовый preview
    этого tool может содержать только первые совпадения. Это не
    ...
    target_table, target_field, source_table, source_field, source_layer,
    target_layer и transformation_rule.
    Всегда ищет по всей глобальной таблице без неявного file_id или
