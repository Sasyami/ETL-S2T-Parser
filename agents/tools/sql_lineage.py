...
def parse_sql_column_lineage(
    ...
    """Построить lineage только для выходных колонок явно переданного SQL.

    Строгий контракт: выбирай tool, только когда полный SQL уже дословно есть в
    task, истории или подтверждённом результате предыдущего tool и требуется
    для каждой выходной SELECT-колонки получить ``expression`` и
    ``source_columns``. В ``query`` передавай именно этот SQL; не сочиняй и не
    восстанавливай его по именам таблиц, колонок или описанию трансформации.

    Не выбирай tool для поиска или объяснения JOIN/ON, WHERE, GROUP BY, HAVING,
    ORDER BY, ролей алиасов и любых других произвольных частей SQL: после
    получения SQL planner должен анализировать такой текст напрямую. Также не
    выбирай его для ``table.column`` без SQL, чтения сохранённого правила,
    табличного lineage или интерактивной визуализации. Tool ничего не выполняет
    и не читает из SQLite/Neo4j. SQLGlot учитывает алиасы, CTE, SELECT,
    INSERT ... SELECT и CREATE ... AS SELECT; неразрешимые источники возвращает
    отдельно.
...
def visualize_sql_lineage(
    ...
    """Построить интерактивный HTML-граф lineage переданного SQL-текста.

    Tool анализирует содержимое аргумента query и не читает сохранённые данные;
    используй его, когда SQL уже получен и требуется его графическая
    визуализация. Это единственный tool для интерактивной визуализации
    конкретного SQL-текста:
    выбирай его сразу вместо parse_sql_column_lineage и никогда не подменяй
    глобальным visualize_s2t_table_graph. Инструмент возвращает фактический
    column_lineage и URL интерактивного HTML-графа, созданного нативным SQLGlot
    GraphHTML/vis.js.
